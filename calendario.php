<?php
// calendario.php — Tela de calendário e reservas
...
<div class="bg-white rounded-2xl shadow p-6 lg:p-8 space-y-6">
  <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
    <div>
      <h1 class="text-2xl font-bold">Reservas — Semana de <strong><?= h($monday->format('d/m/Y')) ?></strong> a <strong><?= h($friday->format('d/m/Y')) ?></strong></h1>
    </div>
   <?php
...
<div class="cal-nav">
  <a class="nav-today" href="?<?= h($qsBase) ?>&week=<?= h((new DateTime('today'))->format('Y-m-d')) ?>">
    <!-- ícone calendário (inline SVG, leve) -->
...
<!-- Calendário -->
<main class="bg-white rounded-2xl shadow p-4 lg:p-6">
  <div>
      ...
      <tbody>
        <?php for ($h = 8; $h < 18; $h++): ?>
          <tr class="border-t border-slate-100">
            <td class="px-4 py-4 text-sm text-slate-600 sticky left-0 bg-white"><?= hh($h) ?></td>
            <?php foreach ($weekDays as $d): ?>
              <?php
              ...
              <td class="align-top px-3 py-4">
                <div class="min-h-[72px] flex flex-col gap-3 <?= !$slotFull ? 'cursor-pointer hover:bg-slate-50' : 'opacity-60' ?> rounded-lg p-1"
                  ...
                  <?php else: ?>
                    <span class="self-end text-slate-300 cursor-not-allowed" title="Horário indisponível (salas ocupadas/bloqueadas)">—</span>
                  <?php endif; ?>
